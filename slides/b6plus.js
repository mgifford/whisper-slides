...
/* insertFooter -- append CivicActions footer (logo + values) to each slide */
function insertFooter()
{
  var slides = document.getElementsByClassName('slide');
  var i, slide, footer, logo, text;

  for (i = 0; i < slides.length; i++) {
    slide = slides[i];
    if (slide.classList.contains('no-footer')) continue;
    if (slide.getElementsByClassName('ca-footer').length > 0) continue;

    footer = document.createElement('div');
    footer.setAttribute('class', 'ca-footer');
    logo = document.createElement('span');
    logo.setAttribute('class', 'ca-footer-logo');
    logo.setAttribute('aria-hidden', 'true');
    text = document.createElement('span');
    text.setAttribute('class', 'ca-footer-text');
    text.textContent = "Balance | Openness | Care";
...
function initialize()
{
  ...
  insertFooter();		// Add CivicActions footer to all slides
